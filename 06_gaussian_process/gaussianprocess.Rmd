...
```
```{r}
library(modules) #klmr/modules on github
...
dat<-mget(c("xtrain","xtest","N1","N2","ytrain"))
```

Run the Stan model

```{r}
stmod<-stan_model(file="gaussianprocess.stan")
system.time(stfit<-sampling(stmod,data=dat))
ypred<-summary(stfit,pars="ytest")$summary[,c("mean","2.5%","97.5%")]
pairs(stfit,pars=c("beta","beta0","eta_sq","inv_rho_sq","sigma_sq"))
```
...
```{r}
summary(stfit,pars=c("beta","beta0","eta_sq","inv_rho_sq","sigma_sq"))$summary[,c("mean","2.5%","97.5%")]
```